    ...
    <div class="space-y-6">
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
            ...
            <div class="bg-gray-50 rounded-xl p-4">
                @if(str_ends_with($registration->payment_proof_path, '.pdf'))
                    <a href="{{ asset('storage/' . $registration->payment_proof_path) }}" target="_blank"
                       class="inline-flex items-center gap-2 px-4 py-2 bg-primary/10 text-primary rounded-lg text-sm font-semibold hover:bg-primary/20 transition">
                ...
                @else
                    <a href="{{ asset('storage/' . $registration->payment_proof_path) }}" target="_blank">
                        <img src="{{ asset('storage/' . $registration->payment_proof_path) }}" alt="Bukti Transfer"
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
            ...
            <form method="POST" action="{{ route('admin.registrations.update', $registration) }}" enctype="multipart/form-data">
                @csrf
                @method('PATCH')
                <div class="space-y-4">
                    <div>
                    ...
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-1">Sertifikat (PDF/JPG/PNG, maks 5MB)</label>
                        @if($registration->certificate_path)
                            <div class="mb-2 flex items-center gap-3">
                                <a href="{{ asset('storage/' . $registration->certificate_path) }}" target="_blank" class="text-sm text-primary font-semibold hover:underline">Lihat file saat ini</a>
                                <label class="text-xs text-gray-500 inline-flex items-center gap-1"><input type="checkbox" name="remove_certificate" value="1" class="rounded"> Hapus</label>
                            </div>
                        @endif
